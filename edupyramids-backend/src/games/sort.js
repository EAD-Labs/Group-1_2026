...
module.exports = {
  ...
  offlineKey(game) {
    ...
    return {
      bucket: Object.fromEntries(items.map((it, i) => [token(game.id, `i${i}`), it.bucket])),
      explanation: Object.fromEntries(items.map((it, i) => [token(game.id, `i${i}`), it.explanation ?? null])),
    };
  },
